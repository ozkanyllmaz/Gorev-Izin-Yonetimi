...
const AdminPanel = () => {
    ...
    const exportToExcel = (data) => {
        const mappedData = data.map(item => ({
            ...
            "Durum": statusLabels[item.statusType],
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (!tasksRes.ok || !leavesRes.ok || !usersRes.ok) throw new Error('Sunucudan veri alınamadı');
    ...
    const handleApprove = async (leaveId) => {
        ...
        try {
            const res = await fetch(`${baseUrl}/Leave/approve/${leaveId}`, {
                method: 'PUT',
                credentials: 'include',
            });
    ...
    const handleReject = async (leaveId) => {
        setActionLoading(true);
        try {
            const res = await fetch(`${baseUrl}/Leave/reject/${leaveId}`, {
                method: 'PUT',
                credentials: 'include',
            });
        ...
        <div className='p-6 bg-sky-50 min-h-screen'>
            ...
            <section className='p-6 bg-white rounded-md shadow-xl'>
                ...
                <div className='filters mb-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4'>
                    ...
                    <div className='mt-1 mb-1 flex gap-4' >
                        <button onClick={handleResetFilter} className='font-medium text-sm px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-400 hover:text-white border border-gray-300 shadow-sm'>Filtreleri Sıfırla</button>
                        <button onClick={() => exportToExcel(leaves)}
                            className='flex items-center justify-center border border-gray-300 rounded-md bg-green-500 text-white font-medium shadow-sm hover:bg-green-600 px-4 py-2'>
                ...
                <table className='w-full border border-gray-300'>
                    <thead className='bg-gray-100'>
                        <tr>
                            ...
                            <th className='border p-2 text-left w-48'>İşlem</th>
                        </tr>
                    </thead>
                    <tbody>
                        {leaves.length > 0 ? (
                            leaves.map(leave => (
                                <tr key={leave.id} className='hover:bg-gray-50'>
                                    ...
                                    <td className='border p-2'>{statusLabels[leave.statusType] || leave.statusType}</td>
            ...
            {/** Tum userlar */}
            <section className='p-6 bg-white rounded-md shadow-2xl mt-10'>
                <h2 className='font-semibold text-xl mb-4'>Çalışanlar</h2>
                <table className='w-full border border-gray-300'>
                    <thead className='bg-gray-50'>
                        <tr>
                            <th className='border p-2 text-left w-64'>Ad Soyad</th>
                    ...
                    <tbody>
                        {users && users.length > 0 ? (
                            ...
                            <tr><td colSpan="7" className='border p-2 text-center'>Kullanıcı bulunamadı</td></tr>
                        )}
                    </tbody>
                </table>
                <div className="flex justify-between mt-4">
